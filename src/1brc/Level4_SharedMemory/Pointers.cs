namespace Level4_SharedMemory;

/// <summary>
/// The six sections of "Working with Pointers in C#", run with
/// <c>dotnet run --project Level4_SharedMemory -c Release -- pointers</c>.
/// </summary>
public static class Pointers
{
    public static void Run()
    {
        Console.WriteLine("=== Level 4: Working with Pointers in C# ===");
        Console.WriteLine();
        ...
        Section("4 — Pointer Arithmetic: sizeof and Address Difference");
        {
            unsafe
            {
                ...
                fixed (char* ptr = s)
                {
                    ...
                    Console.WriteLine($"\n  &s[0] = 0x{(nint)pA:X}");
                    Console.WriteLine($"  &s[3] = 0x{(nint)pD:X}");
        ...
        Section("5 — String Mutation: Modifying with Pointer (Dangerous!)");
        {
            ...
            // CA1837/CA1845 style analyzers do not fire here, but the course's
            // `new string("Harmless")` overload does not exist; new string(span) is the
            // equivalent non-interned copy.
            string s = new string("Harmless".AsSpan());   // new string() → non-interned copy
        ...
        Section("6 — ReadOnlySpan<char> vs Pointer: Modern Alternative");
        {
            ...
            {
                fixed (char* ptr = span)
                    Console.WriteLine($"  span ptr   : 0x{(nint)ptr:X}  (same address as s's first char)");

                fixed (char* ptr = s)
                    Console.WriteLine($"  s    ptr   : 0x{(nint)ptr:X}");
            }
