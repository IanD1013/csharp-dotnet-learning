...
// GlobalConstants.FilePath is static readonly here (it resolves under the temp folder), so this
// cannot be a const the way it is in the course repo.
var defaultOutputPath = GlobalConstants.FilePath;
...
// The course types both answers at the prompt. Args win when they are supplied, so the generator
// is also usable non-interactively: `dotnet run -c Release -- 10_000 [path]`.
Console.Write("Row Count: ");
var rowInput = args.Length > 0 ? args[0] : Console.ReadLine();
...
var pathInput = args.Length > 1 ? args[1] : Console.ReadLine();
