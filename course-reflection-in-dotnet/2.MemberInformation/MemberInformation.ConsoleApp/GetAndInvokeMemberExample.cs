...
public sealed class GetAndInvokeMemberExample
{
    public void Run()
    {
        BindingFlags searchBindingFlags = BindingFlags.Public | BindingFlags.Instance;
        Type type = typeof(MyType);

        var myConstructorMember = type.GetMember(".ctor", searchBindingFlags)
            .Single();
        Console.WriteLine($"My Constructor Member: {myConstructorMember.Name}");

        var myMessageMember = type.GetMember("MyMessage", searchBindingFlags)
            .Single();
        Console.WriteLine($"My Message Member: {myMessageMember.Name}");

        var myMethodMember = type.GetMember("MyMethod", searchBindingFlags)
            .Single();
